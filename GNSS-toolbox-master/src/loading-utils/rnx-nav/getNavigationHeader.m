...
while 1
    ...
    if contains(line,'IONOSPHERIC CORR') && contains(line, 'GPSA')
        hdr.ionoAlpha = cell2mat(textscan(line(5:60),'%f %f %f %f'));
    end   
    
    if contains(line,'IONOSPHERIC CORR') && contains(line, 'GPSB')
        hdr.ionoBeta = cell2mat(textscan(line(5:60),'%f %f %f %f'));
    end  
    
    if contains(line,'TIME SYSTEM CORR') && contains(line, 'GPUT')
        hdr.timeCorr = sscanf(line(5:60),'%f');
    end  
